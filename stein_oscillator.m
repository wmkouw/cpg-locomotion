...
coupling = -0.2; % Inhibitory coupling strength
...
for t = 2:length(time)
    for i = 1:num_neurons
        % Compute the driving signal fi with coupling
        fi = f * (1 + k1 * sin(k2 * time(t))) + sum(inhibition_matrix(i, :) .* x(:, t-1)');
        
        % Safe computation of the adaptation term
        adaptation = exp(max(-50, min(50, -b * (y(i, t-1) - z(i, t-1)))));  % Prevent overflow
        
        % Update equations
        dx = dt * (a - x(i, t-1) + fi + adaptation);
